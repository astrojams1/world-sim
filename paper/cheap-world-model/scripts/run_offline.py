...
"""Run the sandbox helper's automatic pipeline (solve_all, no LLM) on rendered rooms.
  ...
  python3 run_offline.py --rooms experiments/rooms --filter "^(10[1-9]|110)$" --out experiments/offline-default.json
...
def main():
    ...
    with ThreadPoolExecutor(a.parallel) as ex:
        rows = list(ex.map(lambda d: run_room(os.path.join(a.rooms, d)), dirs))
    for r in rows:
