#!/usr/bin/env node
/**
 * Render benchmark rooms with the app (no API calls) and save the two camera feeds + ground truth.
 *   node paper/cheap-world-model/scripts/render_rooms.mjs --seeds 101-110,201-210 [--objects 8] --out paper/cheap-world-model/experiments/rooms
 * Requires `npm run dev` on http://localhost:3000 and the pre-installed chromium (CHROMIUM_PATH).
 * Rooms are named <seed> or <seed>-o<N>; each gets camera_A.jpg, camera_B.jpg, truth.json.
 */
import fs from "node:fs";
import path from "node:path";
import { chromium } from "playwright";

const args = Object.fromEntries(process.argv.slice(2).reduce((acc, a, i, arr) => {
  if (a.startsWith("--")) acc.push([a.slice(2), arr[i + 1] && !arr[i + 1].startsWith("--") ? arr[i + 1] : "true"]);
  return acc;
}, []));
const seeds = (args.seeds ?? "101-110").split(",").flatMap((s) => {
  const [a, b] = s.split("-").map(Number);
  return b ? Array.from({ length: b - a + 1 }, (_, i) => a + i) : [a];
});
const objects = args.objects ? Number(args.objects) : null;
const out = args.out ?? "paper/cheap-world-model/experiments/rooms";
const url = args.url ?? "http://localhost:3000/";

const browser = await chromium.launch({ executablePath: process.env.CHROMIUM_PATH || undefined, args: ["--use-gl=swiftshader", "--enable-webgl", "--ignore-gpu-blocklist"] });
const page = await browser.newPage({ viewport: { width: 1400, height: 1000 } });
await page.goto(url, { waitUntil: "networkidle" });
await page.waitForSelector('img[alt="Camera A"]', { timeout: 60000 });
let sinceReload = 0;
for (const seed of seeds) {
  // The headless WebGL context goes black after roughly 14 renders in one page; reload every 6 seeds.
  if (sinceReload >= 6) {
    await page.reload({ waitUntil: "networkidle" });
    await page.waitForSelector('img[alt="Camera A"]', { timeout: 60000 });
    sinceReload = 0;
  }
  sinceReload++;
  const seedInput = page.locator("input").first();
  await seedInput.fill(String(seed));
  await seedInput.press("Enter");
  await page.waitForTimeout(700);
  if (objects !== null) {
    await page.selectOption('select[aria-label="Objects"]', String(objects));
    await page.waitForTimeout(700);
  }
  const feeds = await page.evaluate(() => ({
    A: document.querySelector('img[alt="Camera A"]').src,
    B: document.querySelector('img[alt="Camera B"]').src,
    truth: [...document.querySelectorAll("pre")].map((p) => p.textContent).find((t) => t.trim().startsWith('{\n  "objects"')),
  }));
  const name = objects === null ? String(seed) : `${seed}-o${objects}`;
  const dir = path.join(out, name);
  fs.mkdirSync(dir, { recursive: true });
  for (const cam of ["A", "B"]) {
    fs.writeFileSync(path.join(dir, `camera_${cam}.jpg`), Buffer.from(feeds[cam].split(",")[1], "base64"));
  }
  fs.writeFileSync(path.join(dir, "truth.json"), feeds.truth);
  if (feeds.A.length < 4000) throw new Error(`black render for ${name}: WebGL context lost`);
  console.log(name, JSON.parse(feeds.truth).objects.length, "objects");
}
await browser.close();
